...
public class UsersController : ApiControllerBase {
    ...
    public async Task<ActionResult> AssignSupervisor(Guid employeeId, [FromBody] Guid? supervisorId,
        CancellationToken cancellationToken) {
        var result = await Mediator.Send(new AssignSupervisorCommand(employeeId, supervisorId), cancellationToken);
        return HandleResult(result);
    }

    /// <summary>Resets an employee's password.</summary>
    [HttpPost("{id:guid}/reset-password")]
    public async Task<ActionResult> ResetPassword(Guid id, [FromBody] ResetPasswordRequestBody body,
        CancellationToken cancellationToken) {
