...
public class GetHoursReportQueryHandler : IRequestHandler<GetHoursReportQuery, Result<HoursReportDto>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly ICurrentUserService _currentUser;

    public GetHoursReportQueryHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUser)
    {
    ...
    public async Task<Result<HoursReportDto>> Handle(GetHoursReportQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            scopeUsers = await _unitOfWork.Users.GetAllAsync(cancellationToken);
        }
        else
        {
            ...
            if (scopeUsers.All(u => u.Id != _currentUser.UserId) && request.UserId != _currentUser.UserId)
            {
    ...
    private static string GetPeriodKey(DateOnly date, ReportGrouping grouping) => grouping switch
    {
        ...
        ReportGrouping.Week => $"{ISOWeek.GetYear(date.ToDateTime(TimeOnly.MinValue))}-W{ISOWeek.GetWeekOfYear(date.ToDateTime(TimeOnly.MinValue)):D2}",
