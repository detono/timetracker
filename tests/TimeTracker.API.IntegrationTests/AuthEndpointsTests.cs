...
public class AuthEndpointsTests : IClassFixture<CustomWebApplicationFactory> {
    ...
    public async Task EmployerCreatesUser_ThenNewUserLogsIn_ReturnsToken() {
        ...
        request.Content =
            JsonContent.Create(new RegisterUserCommand("Test", "User", email, "Password123!", UserRole.Employee));
    ...
    public async Task CreateUser_AsNonEmployer_ReturnsForbidden() {
        ...
        createRequest.Content =
            JsonContent.Create(new RegisterUserCommand("Test", "Employee", email, "Password123!", UserRole.Employee));
        await _client.SendAsync(createRequest);

        var loginResponse =
            await _client.PostAsJsonAsync("/api/auth/login", new { Email = email, Password = "Password123!" });
        ...
        forbiddenRequest.Content = JsonContent.Create(
            new RegisterUserCommand("Nope", "Blocked", $"{Guid.NewGuid()}@test.local", "Password123!",
                UserRole.Employee));
