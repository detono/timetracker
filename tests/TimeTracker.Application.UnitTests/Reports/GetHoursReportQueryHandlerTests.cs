...
public class GetHoursReportQueryHandlerTests {
    ...
    public async Task Handle_EmployerGroupedByDay_AggregatesHoursPerEmployeePerDayPerType() {
        ...
        _timeEntryRepository
            .Setup(r => r.GetForUsersAsync(It.IsAny<IReadOnlyCollection<Guid>>(), from, to,
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<TimeEntry> {
                TimeEntry.Create(employee.Id, _workType.Id, from, new TimeOnly(9, 0), new TimeOnly(17, 0)), // 8h
                TimeEntry.Create(employee.Id, _workType.Id, from, new TimeOnly(18, 0),
                    new TimeOnly(19, 0)), // 1h same day
                TimeEntry.Create(employee.Id, _workType.Id, to, new TimeOnly(9, 0), new TimeOnly(13, 0)) // 4h next day
            });

        var handler = new GetHoursReportQueryHandler(_unitOfWork.Object,
            new TestCurrentUserService(employer.Id, UserRole.Employer));
        var result =
            await handler.Handle(new GetHoursReportQuery(from, to, ReportGrouping.Day), CancellationToken.None);
    ...
    public async Task Handle_MixedHourTypes_ProducesSeparateLinesForSamePeriod() {
        ...
        // FIX 2: Provide the localized names dictionary
        var sickType = HourType.Create(
            new Dictionary<string, string> { { "en", "Sick Leave" } }, "#b3452f", true
        );
        ...
        _timeEntryRepository
            .Setup(r => r.GetForUsersAsync(It.IsAny<IReadOnlyCollection<Guid>>(), day, day,
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<TimeEntry> {
                TimeEntry.Create(employee.Id, _workType.Id, day, new TimeOnly(9, 0), new TimeOnly(13, 0)), // 4h Work
                TimeEntry.Create(employee.Id, sickType.Id, day, new TimeOnly(13, 0),
                    new TimeOnly(17, 0)) // 4h Sick Leave
            });

        var handler = new GetHoursReportQueryHandler(_unitOfWork.Object,
            new TestCurrentUserService(employer.Id, UserRole.Employer));
        var result =
            await handler.Handle(new GetHoursReportQuery(day, day, ReportGrouping.Day), CancellationToken.None);
