...
export const projectsApi = {
    getAll: async (includeInactive = false): Promise<Project[]> => {
        const response = await apiClient.get<Project[]>(`/api/projects?includeInactive=${includeInactive}`);
        return response.data;
    },

    create: async (payload: { name: string; clientName: string | null }): Promise<string> => {
        const response = await apiClient.post<string>("/api/projects", payload);
        return response.data;
    },
    ...
    toggleStatus: async (id: string, isActive: boolean): Promise<void> => {
        await apiClient.patch(`/api/projects/${id}/status`, isActive);
    }
};